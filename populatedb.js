...
async function authorCreate(index, first_name, family_name, d_birth, d_death) {
  ...
  const author =  Author(authordetail);
...
async function bookInstanceCreate(index, book, imprint, due_back, status) {
  ...
  const bookinstance =  BookInstance(bookinstancedetail);
...
async function createBooks() {
  ...
  await Promise.all([
    ...
    bookCreate(4,
      "Death Wave",
      "In Ben Bova's previous novel  Earth, Jordan Kell led the first human mission beyond the solar system. They discovered the ruins of an ancient alien civilization. But one alien AI survived, and it revealed to Jordan Kell that an explosion in the black hole at the heart of the Milky Way galaxy has created a wave of deadly radiation, expanding out from the core toward Earth. Unless the human race acts to save itself, all life on Earth will be wiped out...",
      "9780765379504",
      authors[1],
      [genres[1]]
    ),
...
async function createBookInstances() {
  ...
  await Promise.all([
    ...
    bookInstanceCreate(3,
      books[3],
      " York Tom Doherty Associates, 2016.",
      false,
      "Available"
    ),
    ...
    bookInstanceCreate(6,
      books[4],
      " York, NY Tom Doherty Associates, LLC, 2015.",
      false,
      "Available"
    ),
